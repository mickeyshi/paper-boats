...
// MV.js - The textbook author's example Matrix / Vector math library for Javascript projects.  Whereas for C++ projects, we will use Eigen, a popular matrix library.
//----------------------------------------------------------------------------
//
//  Helper functions
//
...
function skew(x1, x2) //Skew along the x-axis
{
    if ( Array.isArray(x1) && x1.length == 3 ) {
        x2 = x1[2];
        x1 = x1[1];
        x1 = x1[0];
    }
...
function lookAt( eye, at, up )
{
	...
	if( n[0] != n[0] )		n = normalize( subtract( vec3( 1, 1, 1 ), v ) );	// parallel case - handle infinite possible "n" and "u" vectors
