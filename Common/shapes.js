...
{
	shape.call(this);
	// Inherit class shape’s array members by calling parent constructor
	if( !arguments.length) return;
	// Pass no arguments if you just want to make an empty dummy object that inherits everything, for populating other shapes
	this.populate( this, points_transform );
	// Otherwise, a new triangle immediately populates its own arrays with triangle points,
	this.init_buffers();
	// Then sends its arrays to the graphics card into new buffers
}
inherit(triangle, shape);
triangle.prototype.populate = function( recipient, points_transform )
// The meat of the triangle starts here:
{
	 var offset = recipient.vertices.length;
	 var index_offset = recipient.indices.length;
	// Recipient's previous size
	...
	recipient.indices = [ 0, 1, 2 ];
	 for( var i = offset; i < recipient.vertices.length; i++ )
	// Apply points_transform to those points added during this call
	 recipient.vertices[i] = vec3( mult_vec( points_transform, vec4( recipient.vertices[ i ], 1 ) ) );
};
...
// The meat of the triangle starts here:
{
	 ...
	 var index_offset = recipient.indices.length;
	// Recipient's previous size
	recipient.vertices = [ vec3(-1,-1,0), vec3(1,1,0), vec3(1,-1,0), vec3(1,1,0), vec3(-1,-1,0), vec3(-1,1,0) ];
	//recipient.normals  = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) , vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) ];
	recipient.texture_coords  = [ vec2(0,0), vec2(1,1), vec2(1,0) , vec2(1,1), vec2(0,0), vec2(0,1) ];
	recipient.indices = [ 0, 1, 2 , 3 , 4, 5];
	 for( var i = offset; i < recipient.vertices.length; i++ )
     recipient.flat_normals_from_triples( index_offset );
	// Apply points_transform to those points added during this call
	 recipient.vertices[i] = vec3( mult_vec( points_transform, vec4( recipient.vertices[ i ], 1 ) ) );
...
windmill.prototype.populate = function( recipient, points_transform )
 // The meat of the windmill starts here:
 {
 var offset = recipient.vertices.length;
 var index_offset = recipient.indices.length;
 // Recipient's previous size
 ...
 for( var i = 0, i_max = 4; i < i_max; i++ ) {
	 ...
	 recipient.texture_coords.push( vec2( 1, 0 ) );
	 recipient.indices.push ( 0 );     recipient.indices.push ( 2 * i + 1 );
	 recipient.indices.push ( 2 * i + 2 );
 }
 for( var i = offset; i < recipient.vertices.length; i++ )
	 // Apply points_transform to those points added during this call
	 recipient.vertices[i] = vec3( mult_vec( points_transform, vec4( recipient.vertices[ i ], 1 ) ) );
...
function boat( points_transform )// Argument points_transform: Always identity if we’re just building a triangle.  It does good when building other
 // compound shapes by populating them with a pre-transformed triangle
{
	shape.call(this);
	// Inherit class shape’s array members by calling parent constructor
...
inherit(boat, shape);
boat.prototype.populate = function( recipient, points_transform )
// The meat of the boat starts here:
{
	 var offset = recipient.vertices.length;
	 var index_offset = recipient.indices.length;
	// Recipient's previous size
	recipient.vertices = [ vec3(-1,0,0), vec3(0,1,0), vec3(1,0,0) ];
	recipient.normals  = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1) ];
	recipient.texture_coords  = [ vec2(0,0), vec2(0,1), vec2(1,0) ];
	recipient.indices = [0, 1, 2];

	//Map out the x-y values
	
	var currIndex = 2;
	for(var i = -1, i_max = 3; i < i_max; i+=2)
	{
		for(var j = -1, j_max = 3; j < j_max; j+=2)
		{
			//Center spike
            recipient.vertices.push(vec3(0, 2, 0));
			recipient.vertices.push(vec3(0, 0, i*2));
			recipient.vertices.push(vec3(j*3, 0, 0));
            recipient.texture_coords.push( vec2( .5, .5 ) ); 
			recipient.texture_coords.push( vec2( .5+.5*i,.5  ) );    
	 		recipient.texture_coords.push( vec2( .5, .5+.5*j ) );
            currIndex++; 
			recipient.indices.push ( currIndex );     
			currIndex++;
			recipient.indices.push (currIndex);
			currIndex++;
		    recipient.indices.push (currIndex);
		    ...
		    //Perimeter
            recipient.vertices.push(vec3(0, 1, 3*i));
            recipient.vertices.push(vec3(3*j, 0, 0));
			recipient.vertices.push(vec3(4*j, 1, 0));
            recipient.texture_coords.push( vec2( 0, 0 ) ); 
			recipient.texture_coords.push( vec2( Math.sqrt(19)/5, 0 ) );    
	 		recipient.texture_coords.push( vec2( .90023944, .43091 ) );
            currIndex++;
			recipient.indices.push ( currIndex );     
			currIndex++;
			recipient.indices.push (currIndex);
            ...
            recipient.vertices.push(vec3(j*3, 0, 0));
            recipient.vertices.push(vec3(0, 1, i*3));
            recipient.vertices.push(vec3(0, 0, i*2));
            recipient.texture_coords.push( vec2( 0, 0 ) );
            recipient.texture_coords.push( vec2( Math.sqrt(19)/5, 0 ) );
		    recipient.texture_coords.push( vec2( .77658, .39612518 ) );  
            currIndex++;
			recipient.indices.push (currIndex);     
			currIndex++;
			recipient.indices.push (currIndex);
    ...
    recipient.flat_normals_from_triples( index_offset );
    /*
    for(var k = 0; k < recipient.indices.length; k++){
                console.log("vertex:" + recipient.vertices[recipient.indices[k]] + 
                " normal: " + recipient.normals[recipient.indices[k]]);}*/
	 for( var i = offset; i < recipient.vertices.length; i++ )
	// Apply points_transform to those points added during this call
	 recipient.vertices[i] = vec3( mult_vec( points_transform, vec4( recipient.vertices[ i ], 1 ) ) );
};

function paperplane( points_transform )// Argument points_transform: Always identity if we’re just building a plane.  It does good when building other
 // compound shapes by populating them with a pre-transformed plane
{
	shape.call(this);
	// Inherit class shape’s array members by calling parent constructor
	if( !arguments.length) return;
	// Pass no arguments if you just want to make an empty dummy object that inherits everything, for populating other shapes
	this.populate( this, points_transform );
	// Otherwise, a new plane immediately populates its own arrays with triangplanele points,
	this.init_buffers();
	// Then sends its arrays to the graphics card into new buffers
}
inherit(paperplane, shape);
paperplane.prototype.populate = function( recipient, points_transform )
// The meat of the plane starts here:
{
	
	 var offset = recipient.vertices.length;
     var index_offset = recipient.indices.length;
    // Recipient's previous size
    recipient.vertices = [ vec3(2,0,0), vec3(-2.2,0,-.2), vec3(-2,0,-1) ,
        vec3(-2.2,0,-.2), vec3(-2,0,-1),vec3(-2.2,0,-1),
        vec3(2,0,0), vec3(-2.2,0,.2), vec3(-2,0,1) ,
        vec3(-2.2,0,.2), vec3(-2,0,1),vec3(-2.2,0,1),
        vec3(-2.2,0,-.2), vec3(-2.2,-.4,0),vec3(2,0,0),
        vec3(-2.2,0,.2), vec3(-2.2,-.4,0),vec3(2,0,0),
        vec3(-2,0,-1), vec3(-2.2,-.2,-.1),vec3(2,0,0),
        vec3(-2,0,1), vec3(-1,-.2,.1),vec3(2,0,0),
        vec3(-.5,-.25,-.12), vec3(-1,-.2,-.1),vec3(2,0,0),
        vec3(-.5,-.25,.12), vec3(-1,-.2,.1),vec3(2,0,0)];
        
	recipient.normals  = [ vec3(0,1,0), vec3(0,1,0), vec3(0,1,0),
     vec3(0,1,0), vec3(0,1,0), vec3(0,1,0),
     vec3(0,1,0), vec3(0,1,0), vec3(0,1,0),
     vec3(0,1,0), vec3(0,1,0), vec3(0,1,0),
     vec3(-.2,.84 , .4), vec3(-.2,.84 , .4), vec3(-.2,.84 , .4),
     vec3(-.2,.84 , -.4), vec3(-.2,.84 , -.4), vec3(-.2,.84 , -.4),
     vec3(-.2,3.8 , .8), vec3(-.2,3.8 , .8), vec3(-.2,3.8 , .8),
     vec3(-.2,3.8 , -.8), vec3(-.2,3.8 , -.8), vec3(-.2,3.8 , -.8),
     vec3(0.001,.254 , -.55),vec3(0.001,.254 , -.55), vec3(0.001,.254 , -.55),
     vec3(0.001,.254 , .55),vec3(0.001,.254 , .55), vec3(0.001,.254 , .55)];
	recipient.texture_coords  = [ vec2(0,0), vec2(0,1), vec2(1,0),
    vec2(0,0), vec2(0,1), vec2(1,0),
    vec2(0,0), vec2(0,1), vec2(1,0),
    vec2(0,0), vec2(0,1), vec2(1,0),
    vec2(0,0), vec2(2.2,.2), vec2(2.2,0),
    vec2(0,0), vec2(2.2,.2), vec2(2.2,0), 
    vec2(0,0), vec2(2.2,.2), vec2(2.2,0),
    vec2(0,0), vec2(2.2,.2), vec2(2.2,0),
    vec2(0,0), vec2(2.2,.2), vec2(2.2,0),
	...
	for(var currIndex = 0, maxIndices = 24;  currIndex < maxIndices ; currIndex++){
        recipient.indices.push(currIndex);
    }
    recipient.flat_normals_from_triples( index_offset );
    for(var k = 0; k < recipient.indices.length; k++){
...
function paperplane2( points_transform )// Argument points_transform: Always identity if we’re just building a plane.  It does good when building other
 // compound shapes by populating them with a pre-transformed plane
{
	shape.call(this);
	// Inherit class shape’s array members by calling parent constructor
...
inherit(paperplane2, shape);
paperplane2.prototype.populate = function( recipient, points_transform )
// The meat of the plane starts here:
{
     ...
     var index_offset = recipient.indices.length;
    // Recipient's previous size
    recipient.vertices = [ vec3(2,0,-.2), vec3(-3,0,-.2), vec3(-3,0,-1),
    	vec3(2,0,.2), vec3(-3,0,1), vec3(-3,0,.2), 
        vec3(-3,1,-.8), vec3(-3,0,-.8),vec3(-1.75,0,-.8),
        vec3(-3,1,.8), vec3(-3,0,.8),vec3(-1.75,0,.8),
        vec3(-3,0,-.2), vec3(-3,-.4,0),vec3(2,0,-.2),
        vec3(-3,0,.2), vec3(-3,-.4,0),vec3(2,0,.2),
        vec3(2.5,-.4,0), vec3(-3,-.4,0),vec3(2,0,.2),
        vec3(2.5,-.4,0), vec3(-3,-.4,0),vec3(2,0,-.2),
        ];
